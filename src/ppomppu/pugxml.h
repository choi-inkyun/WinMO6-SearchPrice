...
//			charToWide(bufStr,bufWstr);
			MultiByteToWideChar( CP_UTF8, 0, (char*)temp, -1, bufWstr, read_bytes);
			//realloc temp so it will fit the new string
			free(temp);
			temp = (TCHAR*) malloc(sizeof(TCHAR)*tempsize);
			memcpy(temp,bufWstr,read_bytes*sizeof(TCHAR));
...
public:
	...
	//<summary>Cast attribute value as t_string. If not found, return empty.</summary>
	//<returns>The t_string attribute value, or empty.</returns>
	...
	//<summary>Set attribute to t_string.</summary>
	//<param name="rhs">Value t_string to set.</param>
	//<returns>Reference to xml_attribute.</returns>
	xml_attribute& operator=(const t_string& rhs){ value(rhs.c_str()); return *this; }
	...
	//<summary>Right-shift attribute value to t_string.</summary>
	//<param name="rhs">Reference to t_string to set.</param>
	//<returns>Reference to xml_attribute.</returns>
	xml_attribute& operator>>(t_string& rhs)
	{
#ifdef PUGOPT_NONSEG
		rhs.clear();
...
public:
	...
	bool has_attribute(const t_string& name) { return has_attribute(name.c_str()); } //Node has an attribute named 'name'.
#ifdef PUGOPT_NONSEG
...
public:
	...
	xml_attribute attribute(const t_string& name){ return attribute(name.c_str()); }
	//<summary>Access or create the attribute having 'name'.</summary>
	//<param name="name">Name of attribute to access/create.</param>
	//<returns>Reference to xml_attribute wrapper.</returns>
...
public:
	...
	xml_attribute_struct* mapto_attribute_ptr(const t_string& name){ return mapto_attribute_ptr(name.c_str()); }
	...
	xml_node_struct* mapto_child_ptr(const t_string& name){ return mapto_child_ptr(name.c_str()); }
	...
	//<returns>Index of child, or -1 if not found.</returns>
	//<remarks>Implement your own hash table if you have a great many children.</remarks>
	int mapto_child_idx(const t_string& name){ return mapto_child_idx(name.c_str()); }
...
public:
	...
	void all_elements_by_name(const t_string& name,pointer_array& found){ all_elements_by_name(name.c_str(),found); }
	...
	xml_node first_element_by_name(const t_string& name){ return first_element_by_name(name.c_str()); }
	...
	//<remarks>xml_node may be invalid if not found; test with 'empty'.</remarks>
	xml_node first_element_by_value(const t_string& name,const t_string& value){ return first_element_by_value(name.c_str(),value.c_str()); }
	...
	//<returns>Valid xml_node if such element named 'name' is found.</returns>
	//<remarks>xml_node may be invalid if not found; test with 'empty'.</remarks>
	xml_node first_element_by_attribute(const t_string& name,const t_string& attr_name,const t_string& attr_value){ return first_element_by_attribute(name.c_str(),attr_name.c_str(),attr_value.c_str()); }
	...
	//<remarks>If matching co-node was found, '_root' points thereto.</remarks>
	bool moveto_first_sibling(const t_string& name){ return moveto_first_sibling(name.c_str()); }
	...
	//<remarks>If matching sub-node was found, '_root' points thereto.</remarks>
	bool moveto_child(const t_string& name){ return moveto_child(name.c_str()); }
	...
	bool moveto_next_sibling(const t_string& name){ return moveto_next_sibling(name.c_str()); }
	...
	t_string path(const TCHAR* delimiter = _T("/"))
	{
		...
		strcatgrow(&temp,path); //Append current path.
		free(path); //Free the old path.
		t_string returns = temp; //Set path as new string.
		free(temp);
	...
	xml_node first_element_by_path(const t_string& path,const t_string& delimiter = _T("/")){ return first_element_by_path(path.c_str(),delimiter.c_str()); }
...
public:
	...
	//<returns>Success.</returns>
	bool name(const t_string& new_name){ return name(new_name.c_str()); }
	...
	//<returns>Success.</returns>
	bool value(const t_string& new_value){ return value(new_value.c_str()); }
	...
	//<returns>Success.</returns>
	bool remove_attribute(const t_string& name){ return remove_attribute(name.c_str()); }
	...
	xml_attribute append_attribute(const t_string& name,const t_string& value){ return append_attribute(name.c_str(),value.c_str()); }
